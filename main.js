...
const validateCred = cardNumber => {
    ...
    const digits = Array.from(cardNumber).map(Number);
    
    for(let i = digits.length - 2; i >= 0; i -= 2) {
      digits[i] *= 2; // Double every second digit
    ...
    const sum = digits.reduce((acc, digit) => acc + digit, 0);  // Sum up all the digits
      return sum % 10 === 0; // Check if the total sum is a multiple of 10
